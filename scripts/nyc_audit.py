...
with open(f'{BASE}/courts_new_york_city.json') as f:
    courts = json.load(f)
with open(f'{BASE}/parks_new_york_city.json') as f:
    parks = json.load(f)
with open(f'{BASE}/clubs_new_york_city.json') as f:
    clubs = json.load(f)

print(f"Total courts (NYC-scoped): {len(courts['elements'])}")
print(f"Total park polygons (NYC-scoped): {len(parks['elements'])}")
