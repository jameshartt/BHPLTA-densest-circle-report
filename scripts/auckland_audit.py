#!/usr/bin/env python3
"""Analyze Auckland OSM data for the 2.34km circle ground-truth audit."""
import json
import math
...
print(f"Total courts (Auckland-scoped): {len(courts['elements'])}")
print(f"Total park polygons (Auckland-scoped): {len(parks['elements'])}")
...
print(f"\nCourts within circle (Auckland OSM scope only): {len(in_circle)}")
